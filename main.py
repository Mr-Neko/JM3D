...
from data.dataset_3d import customized_collate_fn, zs_collate_fn
...
def get_args_parser():
    ...
    parser.add_argument('--constrative_text', default='super', type=str)
    parser.add_argument('--class_text', default='None', type=str)
    parser.add_argument('--validate_split', default='None', type=str)
    parser.add_argument('--all_image', action='store_true')
...
def main(args):
    ...
    if utils.is_main_process() and args.wandb:
        ...
        wandb.init(project='JM3D', id=wandb_id, config=args, reinit=True)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    metric_names = models.get_metric_names(args.model, args)
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        if args.soft_label:
            ...
            labels_all = utils.all_gather_batch([labels])[0]
            mask = np.zeros((labels.shape[0], labels_all.shape[0]), dtype=np.float32)
            mask_index = []

            for item in labels:
                mask_index.append([i for i,x in enumerate(labels_all) if x == item ])
            for i in range(len(mask_index)):

                mask[i, [mask_index[i]]] = 1
            mask = torch.from_numpy(mask).cuda(args.gpu, non_blocking=True)
        ...
        with amp.autocast(enabled=not args.disable_amp):
            ...
            if args.class_text != 'None':
                outputs['labels'] = labels.cuda(args.gpu, non_blocking=True)
            if args.soft_label:
                outputs['soft_labels'] = mask
...
def cross_retrived(test_loader, model, tokenizer, args=None):
    ...
    with torch.no_grad():
        ...
        image = pil_loader('/root/ULIP/plane.png')
        image = train_transform(image).unsqueeze(dim=0).cuda()
        ...
        for i, (pc, target, target_name) in enumerate(test_loader):
            ...
            pc_all_feature.append(pc_features)
        ...
        pc_all_feature = torch.cat(pc_all_feature, dim=0)
        # target = torch.cat(target_all)
            # cosine similarity as logits
        image_feature = image_feature / image_feature.norm(dim=-1, keepdim=True)
        logits_per_pc = pc_all_feature @ image_feature.t()

        _, indices = logits_per_pc.topk(10, dim=0)

        indices = indices.cpu().numpy()

        
        for j in indices:
            print(target_all[int(j)])
...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    if args.validate_split == 'Medium':
        labels = ['cone', 'cup', 'curtain', 'door', 'dresser', 'glass box', 'mantel', 'monitor', 'night stand', 'person', 'plant', 'radio', 'range hood', 'sink', 'stairs', 'stool', 'tent', 'toilet', 'tv stand', 'vase', 'wardrobe', 'xbox']
    elif args.validate_split == 'Hard':
        labels = ['cone', 'curtain', 'door', 'dresser', 'glass box', 'mantel', 'night stand', 'person', 'plant', 'radio', 'range hood', 'sink', 'stairs', 'tent', 'toilet', 'tv stand', 'xbox']
    with torch.no_grad():
        text_features = []
        for l in labels:
...
def test_zeroshot_3d(args):
    ...
    test_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=True, sampler=None, drop_last=False, collate_fn=zs_collate_fn
    )
