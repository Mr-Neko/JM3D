...
def init_model(args):
    ...
    model = LlavaLlamaForCausalLM.from_pretrained(model_name, low_cpu_mem_usage=False, use_cache=True, torch_dtype=torch.bfloat16).cuda()
...
def generate_outputs(model, tokenizer, input_ids, pcs, stopping_criteria, do_sample=True, temperature=1.0, top_k=50, max_length=2048, top_p=0.95):
    model.eval() 
...
def start_generation(model, tokenizer, conv, dataloader, annos, prompt_index, output_dir, output_file):
    ...
    mm_use_im_start_end = point_backbone_config['mm_use_im_start_end']
    ...
    for batch in tqdm(dataloader):
        pcs = batch["pcs"].cuda().to(model.dtype) # * tensor of B, N, C(3)
        ...
        outputs = generate_outputs(model, tokenizer, input_ids, pcs, stopping_criteria) # List of str, length is B
