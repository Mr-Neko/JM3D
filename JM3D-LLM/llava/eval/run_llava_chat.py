...
# from llava.data import load_objaverse_point_cloud
import numpy as np
import os
...
def init_model(args):
    ...
    model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, use_cache=True, torch_dtype=args.torch_dtype).cuda()
    model.initialize_tokenizer_point_backbone_config_wo_embedding(tokenizer)

    model.eval()
    import pdb
    pdb.set_trace()
    mm_use_point_start_end = getattr(model.config, "mm_use_point_start_end", True)
    # Add special tokens ind to model.point_config
    point_backbone_config = model.get_model().point_backbone_config
    
    if mm_use_point_start_end:
        # if "v1" in model_path.lower() or 'PointLLM' in model_path.lower():
...
def start_conversation(args, model, tokenizer, point_backbone_config, keywords, mm_use_point_start_end, conv):
    ...
    while True:
        ...
        try:
            id, pcs = load_point_cloud(args)
        except Exception as e:
            print(f"[ERROR] {e}")
            continue
        pcs = pcs.cuda().to(args.torch_dtype)
